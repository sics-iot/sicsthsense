...
public class Streams extends Controller {
      ...
      WS.url(url).get().map(
        new Function<WS.Response, Boolean>() {
          public Boolean apply(WS.Response response) {
            ...
            if (parseResponse(endPoint, jsonBody, textBody, resource.path)) {
                logRequest(DataLog.Type.POLL, DataLog.Result.OK, endPoint, endPoint.url, textBody);
            } else {
                logRequest(DataLog.Type.POLL, DataLog.Result.PARSE_ERROR, endPoint, endPoint.url, textBody);
            }
            return true;
          }
        }
  ...
  public static Result post(String userName, String endPointName, String path) {
    ...
    try {
      ...
      if(!parseResponse(endPoint, jsonBody, textBody, path)) {
          logRequest(DataLog.Type.PUSH, DataLog.Result.PARSE_ERROR, endPoint, getRemoteHost(), textBody);
    ...
    logRequest(DataLog.Type.PUSH, DataLog.Result.OK, endPoint, getRemoteHost(), textBody);
    return ok("ok");
  }
  ...
  private static void logRequest(DataLog.Type type, DataLog.Result result, EndPoint endPoint, String host, String textBody) {
      DataLog.add(endPoint, Utils.currentTime(), type, result, host, textBody);
  }
